<template>
  <form @submit.prevent="submit" class="bg-white p-6 rounded-xl shadow mb-6 space-y-4">
    <div>
      ...
      <input type="text" v-model="description" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm"
        required />
    </div>

    <div>
      <label class="block text-gray-700 font-medium mb-1">Énergie utilisée (en kWh)</label>
      <input type="number" v-model.number="energy" min="0"
    ...
    <div>
      ...
      <select v-model="region" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">
